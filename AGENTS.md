...
matches `release/promote-*`, or matches `chore/auto-sync-*`. These are merge-up /
release-promotion PRs that re-package code already reviewed at the feat → dev stage.
Re-reviewing them produces stale, duplicate findings that block the release flow.
