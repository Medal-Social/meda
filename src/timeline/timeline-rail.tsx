  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-3.5 pb-3">
        <ZoomBtn label="−" onClick={() => onZoomChange?.(Math.max(0.2, zoom - 0.4))} />
        <ZoomTrack value={zoom} min={0.2} max={6} />
        <ZoomBtn label="+" onClick={() => onZoomChange?.(Math.min(6, zoom + 0.4))} />
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-border px-3.5 pb-2.5 text-[11px] tabular-nums text-muted-foreground">
        ...
        <span className="ml-auto">
          <strong className="font-semibold text-foreground">{totalCalls}</strong> sessions
        </span>
      </div>

      {/* Scroll area */}
      <div ref={scrollRef} className="relative flex-1 overflow-y-auto overflow-x-hidden pb-16">
        <div className="sticky top-3 z-10 mx-3.5">
          <LiveIndicator now={now} tz={tz} />
        </div>
        <TimelineTape
          now={now}
          events={events}
          pxPerSec={zoom}
          futurePadSec={FUTURE_PAD_SEC}
          pastSpanSec={PAST_SPAN_SEC}
          tz={tz}
          onSelect={onSelect}
        />
      </div>
      ...
      <button
        ...
        className="absolute inset-x-0 bottom-0 z-20 flex items-center justify-center gap-1.5 bg-gradient-to-t from-card via-card/85 to-transparent px-3.5 py-2.5 text-[12px] text-primary hover:text-primary/80"
      >
        <Archive className="size-3.5" /> Manage retention
      </button>
    </aside>
  );
}
...
function ZoomBtn({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex size-5 items-center justify-center rounded bg-muted text-sm leading-none text-muted-foreground hover:text-foreground"
    >
      {label}
    </button>
  );
...
function ZoomTrack({ value, min, max }: { value: number; min: number; max: number }) {
  ...
  return (
    <div
      ...
      aria-valuemax={max}
      aria-valuenow={value}
      className="relative h-1 flex-1 rounded-sm bg-muted"
    >
      <span
        className="absolute -top-[3px] size-2.5 -translate-x-1/2 rounded-full bg-primary shadow-[0_0_0_4px_rgba(137,117,255,0.18)]"
        style={{ left: `${pct}%` }}
      />
    </div>
  );
