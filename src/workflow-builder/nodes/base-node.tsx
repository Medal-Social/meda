...
const HANDLE_CLASS =
  '!w-[11px] !h-[11px] !bg-neutral-400 dark:!bg-neutral-500 !border-2 !border-background !rounded-full';
...
export function BaseWorkflowNode({
  ...
  return (
    <div
      ...
      className={cn(
        'cursor-default select-none rounded-lg border-2 shadow-sm transition-all',
        colors.bg,
        colors.border,
    ...
    >
      ...
      <div className="flex items-start gap-3 p-3">
        <div className={cn('mt-0.5 flex-shrink-0', colors.icon)}>{icon}</div>
        <div className="min-w-0 flex-1">
          <p className="select-none truncate font-medium text-foreground text-sm">{label}</p>
          {description ? (
            <p className="mt-0.5 truncate text-muted-foreground text-xs">{description}</p>
          ) : null}
          {warning ? (
            <p className="mt-1 inline-flex max-w-full rounded-full bg-warning-100 px-2 py-0.5 font-medium text-[10px] text-warning-800 dark:bg-warning-900/30 dark:text-warning-300">
              {warning}
            </p>
          ) : null}
        </div>
      ...
      {(() => {
        ...
        if (showSourceHandle) {
          return <Handle type="source" position={Position.Bottom} className={HANDLE_CLASS} />;
        }
        return null;
      })()}
    </div>
